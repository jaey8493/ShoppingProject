...
public class ProductMgr {
	...
	public ArrayList<ProductBean> getProductAll(){
		...
		try {
			...
			bean.setDetailImage(rs.getString("detailImage"));
			list.add(bean);				
			}
		} catch (Exception e) {
	...
	public boolean insertProduct(HttpServletRequest request) {
		...
		try {
			...
			conn = ds.getConnection();
			String sql = "insert into shop_product(no,name, price, detail, sdate, stock, image, detailImage) values((SELECT MAX(NVL(NO,0))+1 FROM SHOP_PRODUCT),?,?,?,sysdate,?,?,?)"; //now는 sdate(지금) 의미
			pstmt = conn.prepareStatement(sql);
			/*
			 * String sql2 =
			 * "INSERT INTO shop_product(no) VALUES ( SELECT NVL( MAX(no + 1),1 ) FROM shop_product )"
			...
			if(multi.getFilesystemName("detailImage") == null) {
				//신상품 이미지를 택하지 않았을 경우 디폴트 이미지 선택
				pstmt.setString(6, "../images/ready.gif");
			}else {
				pstmt.setString(6,  multi.getFilesystemName("detailImage"));
			}
	...
	public boolean updateProduct(HttpServletRequest request) {
		...
		try {
			...
			if(multi.getFilesystemName("image") == null) {
				...
				String sql ="update shop_product set name=?, price=?, detail=?, stock=?, image=?,detailImage=? where no=?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1,multi.getParameter("name"));
				pstmt.setString(2,multi.getParameter("price"));
				pstmt.setString(3,multi.getParameter("detail"));
